...
int main(int argc, const char** argv) {
  ...
  DataPipelineDataSource<pair<MutDatum<float>, uint32_t>> train_pipeline(
      ...
      static_cast<std::function<pair<MutDatum<float>, uint32_t>(size_t, size_t, pair<Datum, uint32_t>, std::mt19937_64*)>>([](size_t tid, size_t num_tids, auto src_item, auto rng) {
        ...
        /*UniformRandomIsotropicResize resize_cfg = { .lower = 256, .upper = 480 };
        //UniformRandomIsotropicResize resize_cfg = { .lower = 288, .upper = 512 };
        ...
        /*UniformRandomCrop crop_cfg = { .crop_width = 224, .crop_height = 224 };
        //UniformRandomCrop crop_cfg = { .crop_width = 256, .crop_height = 256 };
        TransformImage(crop_cfg, img, rng);*/
        RandomXFlip xflip_cfg;
        XFlipImage(xflip_cfg, img, rng);
        /*//MutDatum<> datum = MutDatum<>::Alloc(3UL * 224UL * 224UL);
        MutDatum<> datum = MutDatum<>::Alloc(3UL * 256UL * 256UL);
